...
def algorithm(draw, grid, start, end):
	...
	while not open_set.empty():
		...
		for neighbor in current.neighbors:
			WHITE_g_score = g_score[current] + 1

			if WHITE_g_score < g_score[neighbor]:
				came_from[neighbor] = current
				g_score[neighbor] = WHITE_g_score
				f_score[neighbor] = WHITE_g_score + h(neighbor.get_pos(), end.get_pos())
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE and start and end:
					...
					possible = algorithm(lambda: draw(win, grid, ROWS, width), grid, start, end)
